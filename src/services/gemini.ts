...
export async function chatWithAI(context: { jobDescription?: string, resume?: string, analysis?: string }, question: string) {
  const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });
  ...
  const prompt = `
    ...
    Previous ATS Analysis: ${context.analysis || 'Not provided'}

    User Question: ${question}

    Command:
    "Provide a helpful, expert career advice response. If context is provided, use it to give specific advice. If context is missing, provide general expert guidance on resumes, ATS, and career growth. Be concise and professional."
  `;
